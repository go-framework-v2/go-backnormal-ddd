...
// GetProjectId 从请求头Header中获取projectId
func GetProjectId(c *gin.Context) (int, error) {
	projectIdStr := c.Request.Header.Get("projectId")
	if projectIdStr == "" {
		err := fmt.Errorf("projectId is empty")
		return 0, err
	}

	projectId, err := strconv.Atoi(projectIdStr)
	if err != nil {
		err = fmt.Errorf("projectId is not a number")
		return 0, err
	}
	if projectId <= 0 {
		err = fmt.Errorf("projectId is not a positive number")
		return 0, err
	}
